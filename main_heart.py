...
class HeartTrajectory:
    """
    Smooth closed heart trajectory in the XY plane.

    The canonical heart is:
        x = s * 16 sin^3(theta)
        y = s * (13 cos(theta) - 5 cos(2theta)
                 - 2 cos(3theta) - cos(4theta))

    It is re-parameterized by arc length so the end-effector does not speed
    up and slow down merely because the analytic heart curve has non-uniform
    parameter speed.

    Z is constant, so the trajectory remains parallel to the top disc.
    """
    def __init__(
        self,
        z,
        ...
        # Translate the canonical heart into the robot's local XY workspace.
        # Shape and scale are unchanged; only the XY origin is moved.
        ...
        self.heart_start = points[0].copy()
    ...
    @property
    def total_time(self):
        return APPROACH_TIME + HEART_TIME

    def _heart_position_by_fraction(self, fraction):
        fraction = float(
    ...
    def position(self, t):
        ...
        if t < APPROACH_TIME:
            ...
            # Quintic smoothstep for zero velocity at both ends.
            a2 = alpha * alpha
            a3 = a2 * alpha
            smooth = (
                10.0 * a3
        ...
        return self._heart_position_by_fraction(u)
...
class LocalHeartCorrector:
    ...
    def correct(
        ...
        if heart_mode:
            ...
            # trajectory. This keeps the correction on the heart curve rather
        ...
        # For the heart, score the actual Cartesian trajectory error plus a
...
class TrajectoryLogger:
    ...
    def log(
        ...
        # For the heart, the old circle radius is not meaningful.
...
def make_summary(
    ...
    heart_errors = errors[circle_mask]
    heart_radial = radial_errors[circle_mask]
    heart_z_errors = z_errors[circle_mask]

    heart_accuracy = accuracy_values[circle_mask]

    mean_accuracy = float(
        np.mean(heart_accuracy)
    )
    ...
    return {
        ...
        "benchmark": {
            ...
            "heart_accuracy_percent": mean_accuracy,
            "passed": bool(
                mean_accuracy >= 92.0
            ),
        },
        ...
        "heart_only": {
            "mean_error_mm": float(
                np.mean(heart_errors)
            ),
            "median_error_mm": float(
                np.median(heart_errors)
            ),
            "rmse_mm": float(
                np.sqrt(
                    np.mean(
...
def main():
    ...
    print("Heart       :", f"{HEART_SCALE * 1000.0:.1f} mm")
    ...
    # Center the heart on the robot's initial XY position so the complete
    # trajectory stays inside the same local workspace used by the circle.
    # The heart remains entirely in XY and therefore stays parallel to the
    # top disc.
    heart_center = initial_tip[:2].copy()

    heart_z = float(
        initial_tip[2]
        + HEART_HEIGHT_ABOVE_TIP
    )

    radial_start = initial_tip.copy()
    radial_start[2] = heart_z
    ...
    config = {
        ...
        "heart": {
            "center_xy_m": heart_center.tolist(),
            "z_m": heart_z,
            "scale_m": HEART_SCALE,
            "scale_m": HEART_SCALE,
            "half_width_mm": 75.0,
        ...
        "stability_profile": {
            "name": "accurate_low_vibration_heart_v1",
        ...
        "plt_heart": {
            "archive_csv": "heart_xy_tracking_YYYYMMDD_HHMMSS.csv",
            "project_csv": "results/heart_xy_tracking.csv",
        },
    ...
    print("  Heart trajectory correction       : ON")
    ...
    #     approach -> heart -> hold
    #
    # heart_xy_tracking.csv is a dedicated plotting file containing ONLY
    # samples from the actual heart phase.  This is important because the
    # approach segment is a straight radial motion and must not be included
    # when plotting the XY heart.
    #
    ...
    with csv_path.open(
        ...
        if "phase" not in reader.fieldnames:
            raise RuntimeError(
                "trajectory.csv is missing the 'phase' column."
            )

        heart_rows = [
            row
            for row in reader
            if str(row["phase"]).strip().lower() == "heart"
        ]

    if len(heart_rows) < 2:
        raise RuntimeError(
            "Not enough heart-phase samples were recorded "
            "to create the XY heart map."
        )

    # Write the exact same heart-only data to both locations expected by
    ...
    print(
        "Heart XY map exported: "
        f"{len(heart_rows)} samples"
    )
    ...
    print(
        f"Heart accuracy        : "
        f"{summary['benchmark']['heart_accuracy_percent']:.3f}%"
    )
    ...
    print(
        f"Heart mean error      : "
        f"{summary['heart_only']['mean_error_mm']:.3f} mm"
    )

    print(
        f"Heart RMSE            : "
        f"{summary['heart_only']['rmse_mm']:.3f} mm"
    )

    print(
        f"Mean cross-track abs error: "
        f"{summary['heart_only']['mean_radial_abs_error_mm']:.3f} mm"
    )

    print(
        f"Mean Z abs error       : "
        f"{summary['heart_only']['mean_z_abs_error_mm']:.3f} mm"
    )
